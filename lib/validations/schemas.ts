...
// Schema de Configuracion de Granja (legacy - para el modelo ConfiguracionGranja)
...
export const registroDiarioSchema = z
  .object({
    ...
    huevosProducidos: z.preprocess(
      toNumber,
      z.number().int().min(0, 'Debe ser mayor o igual a 0')
    ),
    huevosVendidos: z.preprocess(
      toNumber,
      z.number().int().min(0, 'Debe ser mayor o igual a 0')
    ),
    precioVentaUnitario: z.preprocess(
      toNumber,
...
export type ConfiguracionGranjaInput = z.infer<
  typeof configuracionGranjaSchema
>;

...
export default function SetupPage() {
  ...
  const onSubmit: SubmitHandler<SetupInput> = async (data) => {
    ...
    try {
      ...
      await update({ setupCompleto: true, granjaId: json.data.granjaId });
      router.push('/dashboard');
    } catch {
      setError('Error al guardar la granja');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-amber-50 via-orange-50 to-yellow-50 flex items-center justify-center p-4">
      ...
      <div className="w-full max-w-lg">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 mt-1 text-sm">Configura tu granja para empezar</p>
        </div>

        {/* Indicador de progreso */}
        <div className="flex items-center justify-center gap-2 mb-6">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-full bg-amber-500 text-white text-xs font-bold flex items-center justify-center shadow">1</div>
            <span className="text-xs font-medium text-amber-600">Tu granja</span>
          </div>
          <div className="w-8 h-px bg-gray-300" />
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-full bg-gray-200 text-gray-400 text-xs font-bold flex items-center justify-center">2</div>

...
/* ─────────────────────────────────────────
   LOGO UPLOAD — solo visible para ADMIN
───────────────────────────────────────── */
function LogoUpload({ granjaId }: { granjaId: string }) {
  const storageKey = `logo_granja_${granjaId}`;
 
  // ✅ FIX 1 — setState en useEffect eliminado.
  // useState con función lazy lee localStorage UNA vez al montar,
  // sin provocar el ciclo de re-renders que el linter detectaba.
  ...
  const handleRemove = (e: React.MouseEvent) => {
    ...
    // ✅ FIX 2 — bug "no puedo volver a subir después de quitar".
    // El navegador bloquea onChange si el mismo archivo ya fue seleccionado.
    // Limpiar input.value fuerza al browser a tratarlo como una selección nueva.
    if (inputRef.current) {
  ...
  return (
    ...
    >
      ...
      `}>
        {logo ? (
          // ✅ FIX 3 — advertencia de next/image suprimida intencionalmente.
          // El src es siempre un data:base64 generado en el dispositivo del usuario.
          // next/image no puede optimizar URLs de datos base64 (requiere URLs externas
          // o rutas estáticas), por lo que <img> es la opción técnicamente correcta aquí.
      ...
      {/* Overlay cámara al hacer hover sobre logo existente */}
      <div className={`
        absolute inset-0 rounded-2xl bg-black/40 flex items-center justify-center
        transition-opacity duration-200
        ${logo ? 'opacity-0 group-hover:opacity-100' : 'opacity-0'}
      `}>
      ...
      <div className="
        ...
        pointer-events-none transition-opacity duration-150 z-50 hidden lg:block
      ">
...
export default function DashboardLayout({
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* ── Mobile Top Bar ── */}
      <div className="lg:hidden fixed top-0 left-0 right-0 bg-white border-b z-20 px-4 py-3 flex items-center justify-between shadow-sm">
      ...
      {/* ── Sidebar ── */}
      <aside className={`
        fixed top-0 left-0 h-full w-64 bg-white border-r shadow-lg z-30
        transform transition-transform duration-300
        lg:translate-x-0
        ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}
      `}>
 
        {/* Sección superior: logo + nombre granja (solo ADMIN) */}
        {isAdmin ? (
          <div className="p-5 border-b bg-gradient-to-br from-amber-50 to-orange-50">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[10px] text-gray-400 mt-0.5">🐔 AviControl</p>
              </div>
            </div>
          </div>
        ) : (
          <div className="p-6 border-b">
            <h1 className="text-2xl font-bold text-gray-700">🐔 AviControl</h1>
            <p className="text-sm text-gray-500 mt-1">Gestión Avícola</p>
          </div>
        )}

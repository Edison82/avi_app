...
export async function PUT(
  ...
  try {
    ...
    if (session.user.rol !== 'ADMIN') {
      return NextResponse.json({ success: false, error: 'No tienes permisos para editar categorías' }, { status: 403 });
    }
...
// ── PATCH — Activar/Desactivar categoría ─────────────────────
export async function PATCH(
  ...
  try {
    ...
    if (typeof activa !== 'boolean') {
      return NextResponse.json({ success: false, error: 'El campo activa debe ser un booleano' }, { status: 400 });
    }

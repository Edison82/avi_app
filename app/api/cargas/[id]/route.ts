...
// ── PUT — Editar carga, ajusta diferencia en inventario ───────
  ...
  try {
    ...
    const cargaActualizada = await prisma.$transaction(async (tx) => {
      if (categoriaHuevo !== cargaExistente.categoriaHuevo) {
        // Categoría cambió: devolver a la anterior, descontar de la nueva
...
// ── DELETE — Eliminar carga y devolver huevos al inventario ───
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json({
      success: true,
      message: `Carga eliminada. ${carga.huevosEquivalentes} huevos devueltos al inventario.`,
    });
  } catch (error: unknown) {
    const message = error instanceof Error ? error.message : 'Error';
